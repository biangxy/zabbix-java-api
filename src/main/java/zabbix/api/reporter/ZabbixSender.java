...
import zabbix.api.beans.ZabbixRequest;
import zabbix.api.beans.ZabbixRequestData;
import zabbix.api.beans.ZabbixResponse;
...
public class ZabbixSender
{
            ...
            throws UnknownHostException, IOException
    {
        ...
        try
        {
            ...
            if (!currentLine.contains(ZabbixSender.ZABBIX_RESPONSE_HEARDER))
            {
            ...
            response = mapper.readValue(responseString.toString(), ZabbixResponse.class);

            if (response == null || !response.getResponse().trim().toLowerCase()
                            .equals(ZabbixSender.ZABBIX_RESPONSE_SUCCESSFUL_STRING
                                    .toLowerCase()))
            {

                throw new IOException("Zabbix server or proxy do not answer properly");
            }
