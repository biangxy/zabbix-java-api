...
################################### 
#JDK所在路径
#JAVA_HOME=/root/JDK/jdk1.8.0_24
...
#java虚拟机启动参数
JAVA_OPTS=" -Xms256m -Xmx256m -XX:MetaspaceSize=128M -XX:MaxMetaspaceSize=256M -Djava.awt.headless=true"
...
#拼接lib下所有jar
for file in `find  "$APP_HOME/../lib" -name "*.jar"`
do
   #echo  $file
   jars="$jars:$file"
done
...
psid=0
checkpid() {
javaps=`ps ax|grep java|grep $APP_HOME |grep -v grep`
if [ -n "$javaps" ]; then
...
echo "启动 $APP_CLASS ..."
nohup java $JAVA_OPTS -Dlogback.configurationFile=$CONFIG_PATH/logback.xml -classpath $CLASS_PATH $APP_CLASS >> nohup.out 2>&1 &
checkpid
if [ $psid -ne 0 ]; then
...
echo "启动 $APP_CLASS ..."
nohup java $JAVA_OPTS $JAVA_OPTS_JMX -Dlogback.configurationFile=$CONFIG_PATH/logback.xml -classpath $CLASS_PATH $APP_CLASS >> nohup.out 2>&1 &
checkpid
if [ $psid -ne 0 ]; then
echo "程序: $APP_CLASS 启动成功! (进程号=$psid) [OK]"
else
...
echo "调试启动 $APP_CLASS ..."
nohup java $JAVA_OPTS -Dlogback.configurationFile=$CONFIG_PATH/logback.xml -classpath $CLASS_PATH $DEBUG_OPTS $APP_CLASS >> nohup.out 2>&1 &
checkpid
if [ $psid -ne 0 ]; then
